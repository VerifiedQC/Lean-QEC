...
def gen_BB_skeleton(filename, l, m, a1, a2, a3, b1, b2, b3, d):
    s = l * m * 2
    ...
    with open(filename, "w", encoding='utf-8') as f:
        ...
        def x_to_true(f):
            if f.__name__ == "x":
                ...
                return "false"
        f.write(f"  BB_matrix l m ({x_to_true(a1[0])}, {a1[1]}) ({x_to_true(a2[0])}, {a2[1]}) ({x_to_true(a3[0])}, {a3[1]})\n")
        f.write(f"def BB{s}_B :=\n")
        f.write(f"  let l := {l}\n")
        f.write(f"  let m := {m}\n")
        f.write(f"  BB_matrix l m ({x_to_true(b1[0])}, {b1[1]}) ({x_to_true(b2[0])}, {b2[1]}) ({x_to_true(b3[0])}, {b3[1]})\n")
